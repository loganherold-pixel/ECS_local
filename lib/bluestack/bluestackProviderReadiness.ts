...
const READINESS_BY_PROVIDER: Partial<Record<BluestackProvider, BluestackProviderReadinessBase>> = {
  ...
  propane_monitor: {
    ...
    statusLabel: 'Propane sensor linkable',
    statusDetail: 'Propane monitor profiles can link over native BLE. Live tank percentage remains parser-pending until ECS receives a decoded level percentage.',
  ...
  water_monitor: {
    stage: 'native_parser_pending',
    statusLabel: 'Water sensor linkable',
    statusDetail: 'Water and fluid monitor profiles can link over native BLE. Live tank percentage remains parser-pending until ECS receives a decoded level percentage.',
    telemetryTruthLabel: 'Level parser pending',
    connectionPath: 'native_ble',
    requiresNativeBuild: true,
    requiredSecretNames: [],
  ...
  seelevel: {
    stage: 'native_parser_pending',
    statusLabel: 'SeeLevel sensor linkable',
    statusDetail: 'SeeLevel water monitors can link over native BLE. Live tank percentage remains parser-pending until ECS receives a decoded level percentage.',
    telemetryTruthLabel: 'Level parser pending',
    connectionPath: 'native_ble',
    requiresNativeBuild: true,
    requiredSecretNames: [],
