...
const PARSER_PROFILES: Partial<Record<BluestackProvider, Omit<BluestackTelemetryParserProfile, 'provider' | 'displayName'>>> = {
  ...
  propane_monitor: {
    parserId: 'propane_utility_native_ble_pending',
    category: 'propane_monitor',
    domain: 'propane',
    transport: 'ble',
    status: 'native_parser_pending',
    decisionAction: 'link_utility_profile',
    canDecodeLiveTelemetry: false,
    canAttemptLiveConnection: true,
    requiresFieldEvidence: true,
    requiredEvidence: UTILITY_EVIDENCE_REQUIREMENTS,
    notes: 'Propane monitors are recognized and linkable over native BLE, but live tank percentage remains parser-pending until decoded level units are verified.',
  },
  mopeka: {
    parserId: 'mopeka_tank_monitor_native_ble_pending',
    category: 'propane_monitor',
    domain: 'propane',
    transport: 'ble',
    status: 'native_parser_pending',
  ...
  water_monitor: {
    ...
    requiresFieldEvidence: true,
    requiredEvidence: UTILITY_EVIDENCE_REQUIREMENTS,
    notes: 'Water and fluid monitors are recognized and linkable over native BLE, but live tank percentage remains parser-pending until decoded level units are verified.',
  },
  seelevel: {
    parserId: 'seelevel_water_monitor_native_ble_pending',
    category: 'water_tank_monitor',
    domain: 'water',
    transport: 'ble',
    status: 'native_parser_pending',
    ...
    requiresFieldEvidence: true,
    requiredEvidence: UTILITY_EVIDENCE_REQUIREMENTS,
    notes: 'SeeLevel water profiles are recognized and linkable over native BLE, but live tank percentage remains parser-pending until decoded level units are verified.',
  },
};
