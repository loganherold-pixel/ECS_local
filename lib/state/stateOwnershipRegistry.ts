...
export const ECS_STATE_OWNERSHIP_REGISTRY: readonly ECSStateOwnershipRecord[] = [
  ...
  },
  {
    ...
    adapters: [
      ...
      'dispatchOfflineReplayAdapter',
      'dispatchRealtimeAdapter',
      'dispatchCanonicalMigrationCoordinator',
    ],
    locality: 'hybrid',
    persistence: 'hybrid_local_cloud',
    storageKeys: ['ecs_dispatch_persistence', 'canonical Dispatch tables when shadow mode is enabled'],
    schemaVersion: 7,
    hydrationTrigger: 'Optional startup hydration plus Dispatch active-expedition activation',
    hydrationDependencies: ['active_expedition'],
    writeTrigger: 'Permission-validated lifecycle action',
    writeCoalescing: 'Coalesced file writes, stable operation IDs, ordered dependencies, and field-specific deterministic merge rules',
    ...
    migration: 'Local version 7 adds durable Operational Playbook outbox records and conservative Mission outbox recovery while preserving version 6 and all legacy records',
    sensitivity: 'private',
    conflictPolicy: 'manual_resolution',
    retention: '250 Mission Commands, 750 Mission events, 300 offline operations, 100 playbooks with 250 events each, and bounded canonical audit retention',
