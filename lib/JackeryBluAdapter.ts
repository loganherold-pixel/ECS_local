...
export interface JackeryAdapterEventMap {
  connect: { provider: 'jackery'; deviceId?: string | null; timestamp: number };
  connected: { provider: 'jackery'; deviceId?: string | null; timestamp: number };
  disconnect: { provider: 'jackery'; timestamp: number };
  disconnected: { provider: 'jackery'; timestamp: number };
  reconnecting: { provider: 'jackery'; attempt: number; timestamp: number };
  reconnect_start: { provider: 'jackery'; attempt: number; timestamp: number };
  reconnect_success: { provider: 'jackery'; timestamp: number };
  reconnected: { provider: 'jackery'; timestamp: number };
  reconnect_failed: { provider: 'jackery'; attempt: number; timestamp: number };
  telemetry: BluTelemetry;
  data: BluTelemetry;
  error: { provider: 'jackery'; code?: string | null; message: string; timestamp: number };
  status: JackeryECSBridgeState;
}

type JackeryAdapterEventHandler<T extends JackeryAdapterEventName = JackeryAdapterEventName> =
  (payload: JackeryAdapterEventMap[T]) => void;

export interface JackeryECSBridgeState {
  provider: 'jackery';
  connectionState: BluConnectionState;
  isReconnecting: boolean;
  reconnectAttempts: number;
...
class JackeryBluAdapter {
  ...
  // Lifecycle/event bridge state
  private eventListeners = new Map<JackeryAdapterEventName, Set<JackeryAdapterEventHandler>>();
  private lastTelemetry: BluTelemetry | null = null;


  // ── Lifecycle Event Surface ──────────────────────────────────────────

  on<T extends JackeryAdapterEventName>(
    event: T,
    handler: JackeryAdapterEventHandler<T>,
  ): () => void {
    const set = this.eventListeners.get(event) || new Set();
    set.add(handler as JackeryAdapterEventHandler);
    this.eventListeners.set(event, set);
    return () => this.off(event, handler);
  }

  off<T extends JackeryAdapterEventName>(
    event: T,
    handler: JackeryAdapterEventHandler<T>,
  ): void {
    const set = this.eventListeners.get(event);
    if (!set) return;
    set.delete(handler as JackeryAdapterEventHandler);
    if (set.size === 0) {
      this.eventListeners.delete(event);
    }
  }

  addListener<T extends JackeryAdapterEventName>(
    event: T,
    handler: JackeryAdapterEventHandler<T>,
  ): () => void {
  ...
  removeListener<T extends JackeryAdapterEventName>(
    event: T,
    handler: JackeryAdapterEventHandler<T>,
  ): void {
  ...
  subscribeEvent<T extends JackeryAdapterEventName>(
    event: T,
    handler: JackeryAdapterEventHandler<T>,
  ): () => void {
    return this.on(event, handler);
  ...
  getECSBridgeState(): JackeryECSBridgeState {
    const primary = bluDeviceRegistry.getPrimary();
    const primaryDeviceId = primary?.provider === 'jackery' ? primary.device_id : this.getPrimaryDeviceId();
    ...
    return {
      ...
      hasDevices: this.connectedDevices.length > 0 || this.discoveredDevices.length > 0,
      deviceCount: this.connectedDevices.length || this.discoveredDevices.length,
      primaryDeviceId,
  ...
  private emitEvent<T extends JackeryAdapterEventName>(
    event: T,
    payload: JackeryAdapterEventMap[T],
  ): void {
    const handlers = this.eventListeners.get(event);
    if (!handlers || handlers.size === 0) return;
    for (const handler of handlers) {
      try {
        (handler as JackeryAdapterEventHandler<T>)(payload);
      } catch {
  ...
  private emitStatus(): void {
    this.emitEvent('status', this.getECSBridgeState());
  }
    ...
    try {
      ...
      if (Platform.OS === 'web' || !this.isBleAvailable()) {
        if (!isDevMockTelemetryAllowed()) {
          console.log('[BT_LIVE] mock_disabled', { provider: 'jackery', phase: 'scan' });
  ...
  async connect(deviceId?: string): Promise<JackeryConnectResult> {
    ...
    this.notify();
    this.emitEvent('connect', { provider: 'jackery', deviceId: deviceId ?? null, timestamp: Date.now() });
  ...
  async disconnect(): Promise<void> {
    ...
    bluStateStore.setReconnecting(false);
    bluStateStore.clearProviderTelemetry('jackery');
    bluSessionStore.recordDisconnection();

    this.notify();
    this.emitEvent('disconnected', { provider: 'jackery', timestamp: Date.now() });
  ...
  private async attemptQuietReconnect(): Promise<void> {
    ...
    try {
      ...
      this.notify();
      this.emitEvent('reconnect_success', { provider: 'jackery', timestamp: Date.now() });
      this.emitEvent('reconnected', { provider: 'jackery', timestamp: Date.now() });
