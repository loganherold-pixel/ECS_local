...
// Route Confidence Engine v1 is the pure deterministic boundary for trip/route
// readiness. UI surfaces should normalize inputs here and treat unknown,
// unavailable, stale, demo, mock, partial, live, and verified as distinct states.
...
export const ROUTE_CONFIDENCE_DATA_STATES = [
  'unknown',
  'unavailable',
  'stale',
  'demo',
...
export type RouteConfidenceDataState = typeof ROUTE_CONFIDENCE_DATA_STATES[number];
...
export type TripConfidenceSummaryViewModel = {
  ...
  keyWarnings: string[];
  dataConfidence: {
    state: RouteConfidenceDataState;
...
export type RouteConfidenceCategory = TripConfidenceCategory;
export type RouteConfidenceReasonTone = TripConfidenceReasonTone;
export type RouteConfidenceReasonSection = TripConfidenceReasonSection;
export type RouteConfidenceSectionStatus = TripConfidenceSectionStatus;
export type RouteConfidenceRecommendedActionId = TripConfidenceRecommendedActionId;
export type RouteConfidenceEnvironmentInput = TripConfidenceEnvironmentInput;
export type RouteConfidenceTelemetryInput = TripConfidenceTelemetryInput;
export type RouteConfidenceInput = TripConfidenceInput;
export type RouteConfidenceReason = TripConfidenceReason;
export type RouteConfidenceSection = TripConfidenceSection;
export type RouteConfidenceRouteSummary = TripConfidenceRouteSummary;
export type RouteConfidenceRecommendedAction = TripConfidenceRecommendedAction;
export type RouteConfidenceEngineResult = TripConfidenceSummaryViewModel;
...
function dataConfidenceState(args: {
  routeStatus: ExploreRouteTypeStatus;
  providerUnavailable: boolean;
  criticalMissingCount: number;
  routeGeometryMissing: boolean;
  weatherStatus: string;
  daylightStatus: string;
  remotenessStatus: string;
  telemetryStatus: string;
  hasVisibleCaution: boolean;
  visibleWatchCount: number;
}): RouteConfidenceDataState {
  if (args.routeStatus === 'demo_fixture') return 'demo';
  if (args.telemetryStatus === 'mock' || args.telemetryStatus === 'mocked') return 'mock';
  if (
    args.telemetryStatus === 'stale' ||
    args.weatherStatus === 'stale' ||
    args.daylightStatus === 'stale' ||
    args.remotenessStatus === 'stale'
  ) {
    return 'stale';
  }
  if (args.providerUnavailable) return 'unavailable';
  if (args.criticalMissingCount > 0 || args.routeGeometryMissing) return 'partial';
  if (
    args.weatherStatus === 'unknown' ||
    args.weatherStatus === 'unavailable' ||
    args.daylightStatus === 'unknown' ||
    args.daylightStatus === 'unavailable' ||
    args.remotenessStatus === 'unknown' ||
    args.remotenessStatus === 'unavailable'
  ) {
    return 'unknown';
  }
  if (args.hasVisibleCaution || args.visibleWatchCount > 0 || args.routeStatus === 'preview_geometry') return 'partial';
  if (args.routeStatus === 'live_verified_geometry' && args.weatherStatus === 'verified' && args.telemetryStatus === 'live') {
    return 'verified';
  }
  if (
    args.routeStatus === 'live_verified_geometry' ||
    args.weatherStatus === 'live' ||
    args.weatherStatus === 'verified' ||
    args.telemetryStatus === 'live'
  ) {
    return 'live';
  }
  return 'available';
...
export function getTripConfidenceSummary(input: TripConfidenceInput): TripConfidenceSummaryViewModel {
  ...
  if (telemetryStatus === 'live') {
    ...
    score -= 4;
    addReason(reasons, 'mock_telemetry_ignored', 'Mock telemetry not used for confidence', 'caution', 'data');
  } else {
  ...
  const keyWarnings = reasons
    ...
    .slice(0, 5);
  const knownLimitations = reasons
...
export const routeConfidenceLabel = tripConfidenceLabel;
export const evaluateRouteConfidence = getTripConfidenceSummary;
