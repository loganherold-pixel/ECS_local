...
export type GoalZeroAdapterEventName =
  | 'connect'
  | 'connected'
  | 'disconnect'
  | 'disconnected'
...
export interface GoalZeroAdapterEventPayload {
  type: GoalZeroAdapterEventName;
  provider: 'goal_zero';
  timestamp: number;
  state: GoalZeroAdapterState;
  telemetry?: BluTelemetry | null;
  device?: BluDevice | null;
  devices?: BluDevice[];
  error?: string | null;
...
type GoalZeroAdapterEventListener = (payload: GoalZeroAdapterEventPayload) => void;
...
class GoalZeroBluAdapter {
  ...
  private eventSubscribers = new Map<GoalZeroAdapterEventName, Set<GoalZeroAdapterEventListener>>();
  ...
  on(event: GoalZeroAdapterEventName, listener: GoalZeroAdapterEventListener): () => void {
    const listeners = this.eventSubscribers.get(event) ?? new Set<GoalZeroAdapterEventListener>();
    listeners.add(listener);
    this.eventSubscribers.set(event, listeners);
    return () => this.off(event, listener);
  }

  off(event: GoalZeroAdapterEventName, listener: GoalZeroAdapterEventListener): void {
  ...
  addListener(event: GoalZeroAdapterEventName, listener: GoalZeroAdapterEventListener): () => void {
    return this.on(event, listener);
  }

  removeListener(event: GoalZeroAdapterEventName, listener: GoalZeroAdapterEventListener): void {
    this.off(event, listener);
  }

  subscribeEvent(event: GoalZeroAdapterEventName, listener: GoalZeroAdapterEventListener): () => void {
    return this.on(event, listener);
  }

  private emitEvent(
    type: GoalZeroAdapterEventName,
    extras: Omit<Partial<GoalZeroAdapterEventPayload>, 'type' | 'provider' | 'timestamp' | 'state'> = {},
  ): void {
    const listeners = this.eventSubscribers.get(type);
    if (!listeners || listeners.size === 0) return;

    const payload: GoalZeroAdapterEventPayload = {
      type,
      provider: 'goal_zero',
      timestamp: Date.now(),
      state: this.getState(),
  ...
  getECSBridgeState(): {
    ...
    const primaryDevice = primaryDeviceId
      ? bluDeviceRegistry.getDevice('goal_zero', primaryDeviceId) ?? null
      : null;
  ...
  private handlePollFailure(error: string): void {
    ...
    this.emitEvent('error', {
      error,
      meta: { phase: 'poll_failure', consecutiveFailures: this.consecutiveFailures },
    });
    this.emitStatus({ phase: 'poll_failure', consecutiveFailures: this.consecutiveFailures });
  }
  ...
  private async attemptQuietReconnect(): Promise<void> {
    ...
    try {
      ...
      this.emitEvent('reconnect_success', {
        ...
        meta: { attempt: this.reconnectAttempts, provider: 'goal_zero' },
      });
      ...
      this.emitStatus({ phase: 'reconnected' });
  ...
  private handleConnectError(
    ...
    this.emitStatus({ phase: 'connect_error', errorCode: code });
