...
operator_last_verified as (
  select
    ...
    count(*) filter (where last_verified_at is not null)::integer as source_count_with_verification,
    count(*) filter (where last_verified_at is null)::integer as source_count_missing_verification
  from source_summary_rows
)
