...
-- Applied remotely as migration version 20260719001253.
--
...
-- The existing verified_routes_public_recommendation_bbox_idx supplies the
-- ordered latitude/longitude prefix. The explicit latitude lower bound below
-- remains an index condition under generic PL/pgSQL plans; only the very small
-- same-coordinate group requires the ID tie-break filter/incremental sort.
  ...
  loop
    ...
    for candidate in
      ...
      where vr.review_status = 'approved'
        ...
        and vr.center_latitude >= scan_cursor_latitude
        and (
          longitude_delta >= 180
          or (
            not longitude_wraps
