-- Route catalog operator reporting.
-- Adds operator-facing aggregate sections without exposing route geometry.
...
route_totals as (
  select
    ...
    count(*) filter (where last_verified_at is not null)::integer as route_verified_timestamp_count
  from sampled_routes
),
link_window as (
  select
...
source_route_stats as (
  select
    ...
    max(coalesce(l.last_verified_at, r.last_verified_at)) as last_verified_at,
    min(coalesce(l.last_verified_at, r.last_verified_at)) as oldest_verified_at,
    count(distinct r.id) filter (where coalesce(l.last_verified_at, r.last_verified_at) is not null)::integer
      as route_verified_timestamp_count
  from source_rows sr
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'oldestVerifiedAt', oldest_verified_at,
        'routeVerifiedTimestampCount', route_verified_timestamp_count,
...
operator_route_counts_by_source as (
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'providerId', provider_id,
        ...
        'lastVerifiedAt', last_verified_at,
        'oldestVerifiedAt', oldest_verified_at,
        'latestSyncStatus', latest_ingest_status,
        'latestSyncFinishedAt', coalesce(latest_ingest_finished_at, latest_ingest_started_at),
        'latestSyncError', latest_ingest_error_message
      )
      order by route_count desc, provider_id
    ),
    '[]'::jsonb
...
operator_stale_sources as (
  select coalesce(
    jsonb_agg(
      ...
      order by stale_route_count desc, provider_id
    ) filter (where stale_route_count > 0),
    '[]'::jsonb
  ) as value
  from source_summary_rows
),
operator_failed_sync_areas as (
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'providerId', provider_id,
        'name', name,
        'status', coalesce(latest_ingest_status, 'failed'),
        'finishedAt', coalesce(latest_ingest_finished_at, latest_ingest_started_at),
        'errorMessage', coalesce(latest_ingest_error_message, '')
      )
      order by coalesce(latest_ingest_finished_at, latest_ingest_started_at) desc nulls last, provider_id
    ) filter (
      where latest_ingest_id is not null
        and (
          coalesce(latest_ingest_error_message, '') <> ''
          or lower(coalesce(latest_ingest_status, '')) in (
            'failed',
            'failure',
            'error',
            'errored',
...
operator_last_verified as (
  select
    max(last_verified_at) as latest_verified_at,
    min(oldest_verified_at) as oldest_verified_at,
    count(*) filter (where route_count > 0 and last_verified_at is not null)::integer as source_count_with_verification,
    count(*) filter (where route_count > 0 and last_verified_at is null)::integer as source_count_missing_verification,
    count(*) filter (where route_count = 0)::integer as source_count_verification_not_applicable
  from source_summary_rows
)
select jsonb_build_object(
  ...
  'operatorReport', jsonb_build_object(
    'postureTotals', jsonb_build_object(
      'routeCount', (select route_count from route_totals),
      'publicRecommendationCount', (select public_recommendation_count from route_totals),
      'curationOnlyCount', greatest(0, (select route_count from route_totals) - (select public_recommendation_count from route_totals)),
      'needsReviewCount', (select needs_review_count from route_totals),
    ...
    'lastVerified', jsonb_build_object(
      'latestVerifiedAt', (select latest_verified_at from operator_last_verified),
      'oldestVerifiedAt', (select oldest_verified_at from operator_last_verified),
      'sourceCountWithVerification', (select source_count_with_verification from operator_last_verified),
      'sourceCountMissingVerification', (select source_count_missing_verification from operator_last_verified),
      'sourceCountVerificationNotApplicable', (select source_count_verification_not_applicable from operator_last_verified)
    ),
    'routeCountsBySource', (select value from operator_route_counts_by_source),
    'staleSources', (select value from operator_stale_sources),
    'failedSyncAreas', (select value from operator_failed_sync_areas)
  ),
