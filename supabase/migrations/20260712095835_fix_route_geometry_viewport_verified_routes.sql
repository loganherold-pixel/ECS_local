-- The MVUM sync writes canonical geometry to verified_routes, while the first
-- viewport RPC only searched route_segments. Keep both catalog paths visible
-- through the existing RPC contract and index the populated GeoJSON geometry.

alter table public.route_segment_sources
  add column if not exists last_verified_at timestamptz;

create index if not exists verified_routes_route_geometry_viewport_idx
  on public.verified_routes using gist (
    (ST_MakeValid(ST_SetSRID(ST_GeomFromGeoJSON(route_geometry::text), 4326)))
  )
