...
} from '../_shared/routeCatalogColoradoCpwDesignatedTrails.ts';
...
async function upsertRawFeatureRows(
  ...
  for (const chunk of chunkRows(rawFeatureRows)) {
    ...
    if (error) throw new Error(`Unable to batch upsert Colorado CPW raw source features: ${error.message}`);
  }
}
...
async function upsertRouteRows(
  ...
  for (const chunk of chunkRows(routeRows)) {
    const { data, error } = await admin
      ...
      .select('id, public_id');
    if (error) throw new Error(`Unable to batch upsert Colorado CPW route rows: ${error.message}`);
    if (!Array.isArray(data)) throw new Error('Unable to batch upsert Colorado CPW route rows: no rows returned.');
    allRows.push(...data);
  }
...
async function upsertRouteSourceRows(
  ...
  if (sourceRows.length !== sourceRefs.length) {
    throw new Error('Unable to map all Colorado CPW route source rows to route IDs.');
  }
  ...
  for (const chunk of chunkRows(sourceRows)) {
    ...
    if (error) throw new Error(`Unable to batch upsert Colorado CPW route source rows: ${error.message}`);
  }
}

async function fetchColoradoCpwDesignatedTrailGeoJson(maxFeatures: number): Promise<unknown> {
  const queryUrl = new URL(`${COLORADO_CPW_DESIGNATED_TRAILS_SOURCE.sourceUri}/query`);
  queryUrl.searchParams.set('f', 'geojson');
  queryUrl.searchParams.set('where', COLORADO_CPW_DESIGNATED_TRAILS_QUERY.where);
  queryUrl.searchParams.set('outFields', COLORADO_CPW_DESIGNATED_TRAILS_QUERY.outFields);
  queryUrl.searchParams.set('returnGeometry', 'true');
  queryUrl.searchParams.set('outSR', String(COLORADO_CPW_DESIGNATED_TRAILS_QUERY.outSR));
  queryUrl.searchParams.set('resultRecordCount', String(maxFeatures));

  const response = await fetch(queryUrl.toString());
  if (!response.ok) {
    throw new Error(`Colorado CPW designated trails query failed: HTTP ${response.status}`);
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const maxFeatures = Math.max(1, Math.min(500, Math.round(readNumber(body.maxFeatures ?? body.max_features, 150))));
    const now = new Date().toISOString();
    const admin = createAdminClient();

    const { data: source, error: sourceError } = await admin
      .from('route_sources')
      .upsert(coloradoCpwDesignatedTrailsSourceUpsert(now), { onConflict: 'provider_id' })
      .select('id')
      .single();
    if (sourceError) throw new Error(`Unable to upsert Colorado CPW route source: ${sourceError.message}`);
    if (!source) throw new Error('Unable to upsert Colorado CPW route source: no source returned');
    ...
    const { data: ingestRun, error: ingestError } = await admin
      ...
      .insert({
        ...
        metadata: {
          ...
          query: COLORADO_CPW_DESIGNATED_TRAILS_QUERY,
          minMiles,
          maxFeatures,
        },
      })
      .select('id')
      .single();
    if (ingestError) throw new Error(`Unable to start Colorado CPW ingest run: ${ingestError.message}`);
    if (!ingestRun) throw new Error('Unable to start Colorado CPW ingest run: no ingest run returned');

    const payload = await fetchColoradoCpwDesignatedTrailGeoJson(maxFeatures);
    const features = normalizeColoradoCpwDesignatedTrailFeatureCollection(payload);
    ...
    for (const feature of features) {
      const upsert = featureToColoradoCpwDesignatedTrailRouteUpsert(feature, {
        sourceId: source.id,
    ...
    return jsonResponse({
      ok: true,
      source: 'colorado_cpw_designated_trails',
      ...
      caveat: 'Colorado CPW designated trail records are official state source-backed public recommendations with visible warnings. Current closures, permits, trail signage, property rules, weather, fire restrictions, and seasonal conditions still require trip-date checks.',
    });
  } catch (error) {
    console.error('[route-catalog-sync-colorado-cpw-trails]', {
      message: error instanceof Error ? error.message : 'Unknown Colorado CPW sync failure.',
    });
    return jsonResponse({
      ok: false,
      error: error instanceof Error ? error.message : 'Colorado CPW designated trails sync failed.',
    }, 500);
  }
});
