  ...
  try {
    ...
    await admin
      ...
      .update({
        ...
        metadata: { providerId: 'oregon_odf_ohv_gpx', sourceKeys: sources.map((item) => item.key), minMiles, maxTracksPerSource, sources: sourceSummaries, publicRecommendationCount },
      })
      .eq('id', ingestRun.id);
    ...
    return jsonResponse({
      ...
      caveat: 'Oregon ODF OHV GPX records are official state source-backed public recommendations with visible warnings. Current ODF closures, fire restrictions, vehicle class signage, permits, and seasonal conditions still require trip-date checks.',
    });
  } catch (error) {
