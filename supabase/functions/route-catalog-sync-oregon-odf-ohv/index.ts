...
import {
  ...
  type OregonOdfOhvGpxSource,
} from '../_shared/routeCatalogOregonOdfOhv.ts';
...
async function upsertRouteSourceRows(
  ...
  if (sourceRows.length !== sourceRefs.length) {
    throw new Error('Unable to map all Oregon ODF OHV route source rows to route IDs.');
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const sources = selectOregonOdfOhvGpxSources(sourceKeys);
    if (sources.length === 0) {
      return jsonResponse({ ok: false, error: 'At least one valid Oregon ODF OHV GPX source key is required' }, 400);
    }
    ...
    const { data: ingestRun, error: ingestError } = await admin
      ...
      .insert({
        ...
        started_at: now,
        metadata: { providerId: 'oregon_odf_ohv_gpx', sourceKeys: sources.map((item) => item.key), minMiles, maxTracksPerSource },
      })
      .select('id')
      .single();
    ...
    for (const gpxSource of sources) {
      ...
      const tracks = parseOregonOdfOhvGpxTracks(gpxText, gpxSource).slice(0, maxTracksPerSource);
      rawFeatureCount += tracks.length;
      let sourceNormalizedFeatureCount = 0;

      for (const track of tracks) {
        const upsert = gpxTrackToOregonOdfOhvRouteUpsert(track, {
          sourceId: source.id,
    ...
    await admin
      ...
      .update({
        ...
        metadata: { providerId: 'oregon_odf_ohv_gpx', sourceKeys: sources.map((item) => item.key), minMiles, maxTracksPerSource, sources: sourceSummaries, publicRecommendationCount: 0 },
      })
      .eq('id', ingestRun.id);

    return jsonResponse({
      ok: true,
      source: 'oregon_odf_ohv_gpx',
      sourceKeys: sources.map((item) => item.key),
      sources: sourceSummaries,
      rawFeatureCount,
      normalizedFeatureCount,
      publicRecommendationCount: 0,
      caveat: 'Oregon ODF OHV GPX records are official state source inputs for curation only. They do not become public Suggested Routes until current ODF closures, fire restrictions, vehicle class signage, seasonal conditions, and ECS route curation pass.',
    });
  } catch (error) {
    console.error('[route-catalog-sync-oregon-odf-ohv]', {
      message: error instanceof Error ? error.message : 'Unknown Oregon ODF OHV sync failure.',
    });
    return jsonResponse({
      ok: false,
      error: error instanceof Error ? error.message : 'Oregon ODF OHV sync failed.',
    }, 500);
  }
});
