...
async function upsertRawFeatureRows(
  ...
  for (const chunk of chunkRows(rawFeatureRows)) {
    ...
    if (error) throw new Error('Unable to batch upsert Michigan DNR ORV raw source features.');
  }
}
...
async function upsertRouteRows(
  ...
  for (const chunk of chunkRows(routeRows)) {
    const { data, error } = await admin
      ...
      .select('id, public_id');
    if (error || !Array.isArray(data)) throw new Error('Unable to batch upsert Michigan DNR ORV route rows.');
    allRows.push(...data);
  }
...
async function upsertRouteSourceRows(
  ...
  for (const chunk of chunkRows(sourceRows)) {
    ...
    if (error) throw new Error('Unable to batch upsert Michigan DNR ORV route source rows.');
  }
}

async function fetchGpxText(source: MichiganOrvGpxSource): Promise<string> {
  const response = await fetch(source.url);
  if (!response.ok) throw new Error(`Michigan DNR ORV GPX fetch failed for ${source.key}`);
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: source, error: sourceError } = await admin
      ...
      .select('id')
      .single();
    if (sourceError || !source) throw new Error('Unable to upsert Michigan DNR ORV route source');
    ...
    const { data: ingestRun, error: ingestError } = await admin
      ...
      .select('id')
      .single();
    if (ingestError || !ingestRun) throw new Error('Unable to start Michigan DNR ORV ingest run');
