...
The seed layer includes broad model references plus trim/configuration picks for common expedition platforms across trucks, SUVs, crossovers, wagons, and vans. The initial configuration set includes examples such as Tacoma TRD Pro, 4Runner TRD Off-Road, GX 460, Bronco Badlands, Wrangler Rubicon Unlimited, Outback Wilderness, Transit AWD 148 high roof, Sprinter 2500 AWD 144 high roof, Colorado ZR2, Frontier PRO-4X, Tundra TRD Pro, Forester Wilderness, and RAM 2500 Cummins crew 4x4. Catalog stats expose match-level counts so tests can guard that the trim/config layer does not regress back to generic model-only picks.

The profile flow now surfaces "ECS vehicle picks" as trim/configuration-aware candidates while the user is still typing the model. Specific picks can carry `matchLevel` values of `trim` or `configuration`, plus bundled engine, drivetrain, cab, bed, width, track width, turning diameter, and gearing context. Selecting one completes the model/trim/config fields and keeps OEM-backed weight/GVWR as manufacturer-sourced unless the user has already typed a different manual value.

Probable gearing is intentionally gated: catalog candidates may display an axle/gearing label and confidence, but Fleet stores it as confirmed only after the user taps the gearing confirmation action. Unsupported model-year combinations are not silently matched. For example, a 2021+ Ford Bronco can match the modern Bronco reference, while a 2019 Ford Bronco is surfaced as outside the bundled OEM reference window.

Future expansion should add more verified source notes per catalog entry and grow coverage toward additional common pickup, van, SUV, and crossover variants. Do not treat the catalog as a scale ticket or VIN-specific payload authority; door placard and measured weights remain higher confidence.
