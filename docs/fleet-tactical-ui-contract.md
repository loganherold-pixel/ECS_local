...
Fleet may surface compact OEM-reference panels and ECS vehicle picks when year/make/model input can match the offline catalog. Picks should prefer specific model/trim/configuration candidates over generic model matches when bundled data exists, and they must show source/confidence state rather than imply VIN-specific truth. The seed catalog must retain meaningful trim/config coverage across trucks, SUVs, crossovers, wagons, and vans so the profile flow can identify common expedition builds without relying on photos or live provider lookups. The OEM panel must stay data-forward and source-labeled: matched vehicle reference, match level, confidence, verification reminder, fuel capacity, ground clearance, wheelbase, width/height/length where known, front/rear track width where known, off-road angles, turning diameter/radius, and gearing status. Probable gearing must remain visibly unconfirmed until the user confirms it. It must not introduce OEM photos, scraped images, dealer media, or image-heavy vehicle identity. Door placard values, user-entered specs, VIN/OEM matches, and scale tickets remain higher authority than the bundled reference catalog.
