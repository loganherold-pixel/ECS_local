...
Current CampOps status: **Closed field test ready with restrictions; prior risk acceptance expired**.
...
Use `npm run gate:campops-live-readiness:json` before any claim about CampOps live readiness. The current gate reports `closed_field_test_ready` from standard restricted readiness gates. The prior risk-acceptance packet in `docs/campops/closed_field_test_risk_acceptance.md` expired on 2026-06-16 and is intentionally retired; it must not be used to waive evidence gates.

Provider/source output remains shadow-only or unknown unless exact category/region approval is separately recorded. The current restricted path is allowed by visible Android/device QA, guarded privacy/storage approval, source transparency, and no-influence provider posture, not by an active risk waiver.
...
| Provider/source | Source confidence is represented, provider limitations are documented, and target region/category readiness is explicit and approved for influence or accepted as shadow-only/no-influence. | Source confidence and limitations are documented; provider influence is not approved for the target region/category; shadow-only posture is accepted for the restricted APK field test. | Pass for restricted APK field test only; blocks provider influence and broader rollout. |
...
- **APK field-test ready for restricted closed-field validation**: implementation, Android/device QA, privacy/storage, and provider/source shadow-only/no-influence gates pass without relying on the expired risk-acceptance packet.
...
APK field testing may proceed only under the current restricted readiness gates, with source transparency visible, provider influence disabled or shadow-only for unapproved categories, and no precise private coordinates in shared evidence. Any future use of risk acceptance requires a renewed packet with current sign-offs and expiration.
