...
| `npm run gate:mapbox-searchbox-billing` | Passed for instrumentation and report readiness | Navigate destination search, Trip Builder itinerary insert search, Trip Builder route-context places, and Trip Builder Smart Resupply now share Mapbox Search billing instrumentation. The rerunnable invoice-period report is exposed through `npm run report:mapbox-searchbox-billing -- --events=<sanitized-events.json>` and reports Search Box session units, fallback geocode request units, coordinate reuse, optional caller-priced estimates, unlabeled flow failures, and configured spike warnings. |
| `npm run gate:navigate-provider-android-evidence` | Blocked by real provider-backed Android evidence | The Navigate provider sweep harness and blocked manifest path exist at `.smoke/navigate-provider-android-sweep/manifest.json`, but the strict gate must fail until a real sanitized provider summary supplies or references same-run Android candidate pin/action captures, active route-line context, search freeze/standby runtime evidence, and a redacted `logcat_slice`. Old local references cannot satisfy the strict gate. |
...
- Mapbox Search billing protection now produces a per-flow report for real ECS search flows when supplied sanitized billing events. It does not guess contract pricing; dollar estimates appear only when the operator supplies unit costs, while Search Box sessions, fallback geocode requests, and coordinate reuse remain visible as unit counts.
- Provider-backed Navigate candidate pins/actions, active route-line plus provider-candidate context, mobile destination-search freeze/standby runtime evidence, and redacted logcat evidence now have a repeatable harness and blocked manifest at `.smoke/navigate-provider-android-sweep/manifest.json`. The manifest records old local references as reference-only, and old local references cannot satisfy the strict gate; broad rollout remains blocked until the strict gate passes with real sanitized provider-backed Android evidence.
...
1. Re-run Android/device QA with `npm run evidence:navigate-provider-android -- --provider-summary=<sanitized-summary.json> --real`, using a sanitized summary that includes `androidArtifacts` roles for `candidate_pin_visible`, `navigate_here_action`, `save_camp_action`, `report_unusable_action`, `dismiss_action`, `active_route_line_with_provider_candidates`, `search_freeze_standby_runtime`, and `logcat_slice`; then pass `npm run gate:navigate-provider-android-evidence`.
2. Before the next Mapbox monthly invoice review, export sanitized Mapbox Search billing events and run `npm run report:mapbox-searchbox-billing -- --events=<sanitized-events.json> --invoice-period=<label>`. Add `--searchbox-session-unit-cost=<amount>` and `--forward-geocode-unit-cost=<amount>` only when those caller-verified invoice rates are available.
3. Run real provider shadow validation for the target region/category before provider influence.
4. Record provider-specific coverage, freshness, unknown, stale, and conflict rates from real data.
5. Complete broad privacy/storage owner approval before real trip/debrief field-data rollout.
6. Keep AI assist, telemetry, and community publishing disabled unless separate approvals are recorded.
7. Re-run `npm run gate:closed-field-test:json` before any closed-field promotion checkpoint.
