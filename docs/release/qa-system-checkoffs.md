...
| CampOps live readiness | `npm run gate:campops-live-readiness:json` | Passed for internal beta; restricted closed-field posture | Rendering, scoring, safety/copy, privacy/storage implementation, and Android/device QA gates pass. Provider/source influence remains shadow-only/no-influence, not approved for real influence. |
| Closed field test gate | `npm run gate:closed-field-test:json` | Passed with restrictions | Gate reports `ready_with_restrictions`. Provider readiness is not approved for influence; AI assist remains disabled; telemetry and community publishing remain disabled. |
...
- Retired the expired closed-field risk-acceptance packet after its 2026-06-16 expiration; future risk-accepted runs require renewed sign-off.
...
| Closed field-test risk acceptance | Expired / retired | Historical scope has expiration, rollback path, restricted cohort/labels, and explicit risk-accepted items. It no longer waives evidence gates or authorizes new runs. |
