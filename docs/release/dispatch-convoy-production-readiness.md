...
- Sensitive integrations remain default-off: canonical Dispatch persistence, live team position sharing, Convoy Regroup Planner, agency ingestion, public hazard publishing, automated SOS transmission, live radio/network integrations, and demo data.
...
| Canonical Dispatch backend approval | Blocked/default-off | Hosted RLS policy results, isolated two-client convergence, offline replay/reconnect, Android/iOS background behavior, privacy approval, and product/engineering owner acceptance. |
