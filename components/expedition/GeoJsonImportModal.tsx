/**
 * GeoJSON Import Modal — Expedition Command System
 *
 * Full-featured GeoJSON file import with:
 *   - Cross-platform file picker (expo-document-picker native, DOM web)
 *   - RFC 7946 GeoJSON parsing with source app detection
 *   - Feature preview with selective import (per-feature checkboxes)
 *   - Waypoint kind mapping with override UI
 *   - Route geometry passthrough
 *   - ECS metadata round-trip detection
...
import {
  parseGeoJSON, validateGeoJsonFile, mapGeoJsonToWaypointKind,
} from '../../lib/geojsonParser';
import type {
  GeoJsonParseResult, GeoJsonWaypoint, GeoJsonRoute,
} from '../../lib/geojsonParser';
...
const MAX_FILE_SIZE = 20 * 1024 * 1024; // 20 MB (GeoJSON can be larger)
const { width: SCREEN_WIDTH } = Dimensions.get('window');

const GEOJSON_COLOR = '#10B981'; // emerald green for GeoJSON branding
...
export default function GeoJsonImportModal({
  ...
  const handleFileContent = useCallback((name: string, content: string, size: number) => {
    ...
    // Validate
    const validation = validateGeoJsonFile(content);
    if (!validation.valid) {
      setErrorMessage(validation.error || 'Invalid GeoJSON file.');
      setStage('error');
      return;
    }
  ...
  const pickFile = useCallback(async () => {
    if (Platform.OS === 'web' && typeof document !== 'undefined') {
      ...
      input.onchange = (e: any) => {
        ...
        if (file.size > MAX_FILE_SIZE) {
          setErrorMessage(`File too large (${(file.size / (1024 * 1024)).toFixed(1)} MB). Maximum is 20 MB.`);
          setStage('error');
          return;
        }
    ...
    try {
      ...
      const name = asset.name || 'imported.geojson';
      const size = asset.size || 0;

      if (size > MAX_FILE_SIZE) {
        setErrorMessage(`File too large (${(size / (1024 * 1024)).toFixed(1)} MB). Maximum is 20 MB.`);
        setStage('error');
        return;
      }
    ...
    try {
      ...
      if (importWaypoints && selectedWaypoints.size > 0 && importedRouteCount === 0 && createNewRoute) {
        const containerRoute = await routeCommandStore.create(userId, {
          ...
          source: parseResult.source.detectedApp || 'geojson',
        });
        ...
        for (const wpIdx of selectedWaypoints) {
          ...
          await waypointCommandStore.create(userId, {
            ...
            meta: {
              ...
              geojson_symbol: wp.symbol,
              geojson_color: wp.color,
              ...(wp.properties.description ? { description: wp.properties.description } : {}),
            },
          });
        ...
        <View style={s.container}>
          ...
          <View style={s.header}>
            <View style={s.headerLeft}>
              ...
              <View>
                <Text style={s.headerTitle}>GEOJSON IMPORT</Text>
                <Text style={s.headerSub}>
                  {stage === 'pick' ? 'SELECT FILE' :
                   stage === 'preview' ? 'REVIEW FEATURES' :
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={s.body} showsVerticalScrollIndicator={false}>
            ...
            {stage === 'pick' && (
              <View style={s.pickContainer}>
                ...
                <View style={s.appsRow}>
                  {[
                    { name: 'Mapbox', icon: 'map-outline', color: '#4264FB' },
                    { name: 'Leaflet', icon: 'leaf-outline', color: '#199900' },
                    { name: 'QGIS', icon: 'layers-outline', color: '#589632' },
                    { name: 'ArcGIS', icon: 'globe-outline', color: '#2C7AC3' },
                    { name: 'Kepler.gl', icon: 'analytics-outline', color: '#2C51BE' },
                    { name: 'geojson.io', icon: 'globe-outline', color: '#333' },
                    { name: 'D3.js', icon: 'bar-chart-outline', color: '#F9A03C' },
                    { name: 'Turf.js', icon: 'code-outline', color: '#4CAF50' },
                    { name: 'GitHub', icon: 'logo-github', color: '#8A8A85' },
                ...
                <TouchableOpacity style={s.pickBtn} onPress={pickFile} activeOpacity={0.7}>
                  <View style={s.pickBtnIconWrap}>
                    <Ionicons name="code-slash-outline" size={28} color={GEOJSON_COLOR} />
                  </View>
                  <Text style={s.pickBtnTitle}>SELECT GEOJSON FILE</Text>
                  <Text style={s.pickBtnSub}>Browse your device for .geojson or .json files</Text>
                  <View style={{ flexDirection: 'row', gap: 6 }}>
                    <View style={s.pickBtnBadge}>
                      <Text style={s.pickBtnBadgeText}>.GEOJSON</Text>
                    </View>
                    <View style={[s.pickBtnBadge, { backgroundColor: 'rgba(16,185,129,0.08)' }]}>
                      <Text style={s.pickBtnBadgeText}>.JSON</Text>
                    </View>
                  </View>
                </TouchableOpacity>

                <Text style={s.pickNote}>
                  Accepts RFC 7946 GeoJSON files up to 20 MB from any mapping library, GIS tool, or developer workflow
                </Text>
                ...
                <View style={s.infoCard}>
                  ...
                  <View style={s.infoRow}>
                    <Ionicons name="location-outline" size={14} color={GEOJSON_COLOR} />
                    <Text style={s.infoRowText}>Point features as waypoints with kind auto-detection</Text>
                  </View>
                  <View style={s.infoRow}>
                    <Ionicons name="analytics-outline" size={14} color="#42A5F5" />
                    <Text style={s.infoRowText}>LineString / MultiLineString as route geometry (direct passthrough)</Text>
                  </View>
                  <View style={s.infoRow}>
                    <Ionicons name="shapes-outline" size={14} color="#CE93D8" />
                    <Text style={s.infoRowText}>Polygon boundaries as route paths</Text>
                  </View>
                  <View style={s.infoRow}>
                    <Ionicons name="shield-outline" size={14} color="#C48A2C" />
                    <Text style={s.infoRowText}>ECS metadata round-trip (ecs: prefixed properties)</Text>
                  </View>
                  <View style={s.infoRow}>
                    <Ionicons name="color-palette-outline" size={14} color="#FF7043" />
                    <Text style={s.infoRowText}>simplestyle-spec properties (marker-color, stroke, etc.)</Text>
                  </View>
                </View>

                {/* Developer note */}
                <View style={[s.infoCard, { borderColor: 'rgba(16,185,129,0.15)', backgroundColor: 'rgba(16,185,129,0.04)' }]}>
                  <View style={s.infoRow}>
                    <Ionicons name="information-circle-outline" size={14} color={GEOJSON_COLOR} />
                    <Text style={[s.infoRowText, { color: TACTICAL.textMuted, fontSize: 10 }]}>
                      GeoJSON is the native format used by ECS routes. Importing GeoJSON preserves full geometry fidelity with zero conversion loss.
                    </Text>
                  </View>
                </View>
              </View>
              ...
              <View style={s.previewContainer}>
                ...
                <View style={{ flexDirection: 'row', gap: 6, flexWrap: 'wrap' }}>
                  <View style={[s.sourceBadge, { flex: 0, paddingHorizontal: 8, paddingVertical: 4 }]}>
                    ...
                    <Text style={[s.sourceBadgeText, { color: GEOJSON_COLOR, fontSize: 9 }]}>GEOJSON</Text>
                  </View>
                ...
                {parseResult.source.isEcsExport && (
                  <View style={s.ecsRoundTripBadge}>
                    ...
                    <Text style={s.ecsRoundTripText}>
                      ECS EXPORT DETECTED — Waypoint kinds and route metadata will be restored automatically
                    </Text>
                  </View>
                )}
                ...
                <View style={s.statsGrid}>
                  <StatBox
                    label="FEATURES"
                    value={`${parseResult.totalFeatures}`}
                    unit="TOTAL"
                    icon="layers-outline"
                    color={GEOJSON_COLOR}
                  />
            ...
            {stage === 'importing' && (
              <View style={s.importingContainer}>
                <ActivityIndicator size="large" color={GEOJSON_COLOR} />
                <Text style={s.importingTitle}>IMPORTING GEOJSON DATA</Text>
                <Text style={s.importingSub}>Creating routes and waypoints from features...</Text>
            ...
            {stage === 'success' && importStats && (
              <View style={s.successContainer}>
                ...
                <Text style={s.successTitle}>GEOJSON IMPORTED SUCCESSFULLY</Text>
                ...
                <View style={s.successStats}>
                  ...
                  <View style={s.successStatRow}>
                    <Ionicons name="document-outline" size={14} color={GEOJSON_COLOR} />
                    <Text style={s.successStatText}>Source: {fileName}</Text>
                  </View>
                </View>

                <View style={s.successApps}>
                  <Text style={s.successAppsLabel}>GEOJSON NATIVE FORMAT</Text>
                  <Text style={s.successAppsText}>
                    Route geometry preserved with zero conversion loss. Compatible with Mapbox, Leaflet, D3, QGIS, and all GeoJSON consumers.
                  </Text>
                </View>
              </View>
            )}
...
function RoutePreviewRow({ route, index, selected, onToggle }: {
  route: GeoJsonRoute; index: number; selected: boolean; onToggle: () => void;
}) {
  ...
  return (
    <TouchableOpacity style={s.featureRow} onPress={onToggle} activeOpacity={0.7}>
      ...
      <View style={[s.featureIcon, { borderColor: `${color}40` }]}>
        <Ionicons
          name={route.geometry.type === 'Polygon' || route.geometry.type === 'MultiPolygon'
            ? 'shapes-outline' : 'analytics-outline'}
          size={12}
          color={color}
        />
      </View>
      <View style={{ flex: 1 }}>
        ...
        <Text style={s.featureDetail}>
          {route.geometry.type} / {route.pointCount} pts
          {route.distanceMi ? ` / ${route.distanceMi} mi` : ''}
        </Text>
      </View>
      <View style={[s.geomBadge, { borderColor: `${color}40`, backgroundColor: `${color}10` }]}>
        <Text style={[s.geomBadgeText, { color }]}>
          {route.geometry.type === 'Polygon' ? 'POLY' :
           route.geometry.type === 'MultiLineString' ? 'MULTI' : 'LINE'}
        </Text>
      </View>
    </TouchableOpacity>
  );
...
const s = StyleSheet.create({
  ...
  headerTitle: { fontSize: 14, fontWeight: '900', color: GEOJSON_COLOR, letterSpacing: 1.5 },
  ...
  pickBtn: {
    ...
    backgroundColor: 'rgba(16,185,129,0.06)', borderWidth: 1.5,
    borderColor: 'rgba(16,185,129,0.3)', borderStyle: 'dashed', gap: 8,
  },
  pickBtnIconWrap: {
    width: 56, height: 56, borderRadius: 16,
    backgroundColor: 'rgba(16,185,129,0.1)', alignItems: 'center', justifyContent: 'center', marginBottom: 4,
  },
  pickBtnTitle: { fontSize: 14, fontWeight: '900', color: GEOJSON_COLOR, letterSpacing: 1.5 },
  ...
  pickBtnBadgeText: { fontSize: 9, fontWeight: '800', color: GEOJSON_COLOR, letterSpacing: 1 },
  ...
  sectionCheckActive: { backgroundColor: GEOJSON_COLOR, borderColor: GEOJSON_COLOR },
  ...
  toggleBtnText: { fontSize: 8, fontWeight: '800', color: GEOJSON_COLOR, letterSpacing: 1 },
  ...
  featureCheckActive: { backgroundColor: GEOJSON_COLOR, borderColor: GEOJSON_COLOR },
  featureIcon: {
  ...
  optionCheckActive: { backgroundColor: GEOJSON_COLOR, borderColor: GEOJSON_COLOR },
  ...
  importingTitle: { fontSize: 14, fontWeight: '900', color: GEOJSON_COLOR, letterSpacing: 1.5 },
  ...
  importingProgressBar: {
    width: '60%', height: '100%', backgroundColor: GEOJSON_COLOR, borderRadius: 2,
  },
  ...
  successApps: {
    ...
    backgroundColor: 'rgba(16,185,129,0.06)', borderWidth: 1, borderColor: 'rgba(16,185,129,0.15)', width: '100%',
  },
  successAppsLabel: { fontSize: 8, fontWeight: '800', color: TACTICAL.textMuted, letterSpacing: 2, marginBottom: 4 },
  successAppsText: { fontSize: 10, color: GEOJSON_COLOR, textAlign: 'center' },
  ...
  importBtn: {
    ...
    paddingVertical: 14, borderRadius: 12, backgroundColor: GEOJSON_COLOR,
  },
