...
assert(
  !ROUTE_CATALOG_CURATION_COVERAGE_LABELS.includes('BLM GTLF Wyoming') &&
    ...
    !ROUTE_CATALOG_CURATION_COVERAGE_LABELS.includes('Colorado CPW Designated Trails'),
  'Coverage registry should promote official state OHV/NPS/BLM Wyoming sources without leaving official pilots in curation.',
);
assert.deepStrictEqual(
  ROUTE_CATALOG_CURATION_COVERAGE_LABELS,
  [],
  'Resolving BLM Wyoming should leave no official-access source bucket in curation.',
);
assert.match(
  getRouteCatalogCoverageSummary(),
