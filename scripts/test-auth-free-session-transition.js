...
for (const profile of ['fieldtest-pending', 'fieldtest-no-session', 'fieldtest-stale-session', 'qa-supabase-disabled']) {
  ...
  assert.strictEqual(coordinator.requestNavigation(generation), true, `${profile}: navigation dispatches`);
...
assert.strictEqual(coordinator.requestNavigation(generation), true);
...
assert.strictEqual(coordinator.markDestinationMounted(), true, '13: foreground destination remains mounted');
assert.ok(videoOwner.includes('if (disposed) return;'), '14: cleanup is idempotent');
assert.strictEqual(coordinator.requestNavigation(generation), false, '15: navigation count remains exactly one');
...
assert.ok(appContext.includes("event !== 'SIGNED_IN'") && appContext.includes('resetForIntentionalSignIn'), '19: intentional signed-in startup remains authoritative');
...
assert.ok(login.includes('dispatchFreeSessionNavigation(generation)') && layout.includes('committedFreeSession'));
