...
assertIncludes(screen, 'FlatList', 'Offline Prep should use a virtualized content list for mobile scroll performance.');
assertIncludes(screen, 'type OfflinePrepContentRow', 'Offline Prep content rows should be explicit so manifest rows can virtualize independently.');
assertIncludes(screen, 'OFFLINE_PREP_INITIAL_RENDER_COUNT', 'Offline Prep should bound the first mobile render batch.');
assertIncludes(screen, 'renderOfflinePrepContentRow', 'Offline Prep should render route, manifest, and action blocks through a bounded row renderer.');
assertIncludes(screen, 'testID="offline-prep-content-list"', 'Offline Prep should expose the virtualized content list for mobile evidence.');
assertIncludes(screen, "type: 'manifest_item'", 'Offline Prep should render each manifest item as an independent virtualized row.');
