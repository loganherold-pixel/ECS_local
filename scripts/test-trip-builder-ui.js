...
assert(!screen.includes('trip-builder-picker-map-style'), 'Reference pickers should not add a redundant map style control to the focused pin-placement map.');
...
assertIncludes(screen, 'setTripSetupStarted(false);', 'Direct summary handoffs should wait for canonical preparation and trailhead confirmation.');
assertIncludes(screen, "next.status !== 'ready' || !next.canonicalRoute", 'Only restored canonical-ready sessions may auto-open setup.');
assertIncludes(screen, 'setPreparedTripRoutePreview(buildPreparedTripRoutePreview(preparedRoute));', 'Restored canonical detail should prepare the setup preview.');
