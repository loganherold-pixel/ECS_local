...
assertIncludes(screen, 'routeStore.getAll()', 'Trip Builder route selection should include local imported and saved route assets.');
assertIncludes(screen, 'runStore.getAll()', 'Trip Builder route selection should include saved run geometry.');
assertIncludes(screen, 'tripBuilderCatalogMergeInput(liveTrailPackCatalogStore.getSnapshot())', 'Trip Builder route selection should reconcile one authoritative catalog snapshot.');
assertIncludes(screen, 'liveTrailPackCatalogStore.subscribe', 'Trip Builder should reconcile authoritative catalog changes that arrive after route-session hydration.');
assert(!screen.includes('loadOpportunitiesWithCompatibility(null)'), 'Trip Builder route selection must not fall back to discoverEngine fixture routes.');
...
assertIncludes(screen, 'SOURCE-BACKED OR IMPORTED', 'Trip Builder route picker should distinguish reviewed, saved, and imported routes.');
assertIncludes(routePreparation, 'parseNavigateRouteImport', 'Trip Builder imports should reuse the canonical supported-file parser.');
...
assertIncludes(routeImportParser, "format === 'geojson' || format === 'json'", 'The shared route import parser should preserve supported GeoJSON/JSON formats.');
assertIncludes(routeImportParser, 'parseGeoFile(input.fileName, input.content)', 'The shared route import parser should preserve supported GPX/KML formats.');
assertIncludes(screen, 'tripBuilderRouteFromImport', 'Trip Builder should send imported files through canonical route preparation.');
...
assertIncludes(screen, 'buildApproachResupplyStopPlan', 'Trip Builder should derive combined stops from confirmed ranked fuel and supply identities.');
assert(!screen.includes('hasFuelAndGrocerySupport'), 'Trip Builder must not infer grocery coverage from chain-name text alone.');
...
assertIncludes(screen, 'Last Practical Resupply Before Trail Entry', 'Trip Builder should label the operational last-stop recommendation.');
assertIncludes(screen, 'ECS first finds the last reachable stop on the origin-to-entry driving route', 'Trip Builder should explain position-first resupply ranking.');
assertIncludes(screen, 'if (smartResupplyPending) return null;', 'Trip Builder should not show provider-unavailable pre-trail status while approach or POI work is pending.');
assertIncludes(screen, 'preTrailStopCandidatesForDraft, smartResupplyPending, smartResupplyPreference', 'Trip Builder pre-trail status copy should update when deferred/loading resupply work settles.');
...
assertIncludes(screen, 'BEST + 2 ALTERNATIVES', 'Trip Builder should present one best option plus a small alternative set.');
...
assertIncludes(screen, 'within 0.20 miles of the driving approach', 'Trip Builder should disclose the hard normal corridor threshold.');
assertIncludes(screen, 'selectedSmartResupplyStopPlan.stops.map((stop) => {', 'Trip Builder should preserve the canonical approach order for combined or separate selected stops.');
assertIncludes(screen, 'selectedSmartOptions.map((option) => ({ option, point: smartResupplyPointForPlan(option) }))', 'Trip Builder should pass each canonical combined-or-separate selection into the planner once.');
...
assertIncludes(screen, 'pinMarkers={[...routeEndpointMarkers, ...operatorPinMarkers, ...selectedMarker]}', 'Bailout picker should render route endpoints and dropped pins on the interactive map.');
assertIncludes(screen, 'pinMarkers={[...suggestedCampMarkers, ...campMarkers]}', 'Camp picker should render established and operator camp pins on the interactive map.');
...
assertIncludes(screen, 'TRIP_BUILDER_PLANNING_MAP_STYLES', 'Reference pickers should use the canonical planning map-style choices.');
assertIncludes(screen, 'testID="trip-builder-planning-map-style-selector"', 'Camp and bailout pickers should expose the requested Day/Satellite selector.');
...
assertIncludes(screen, "| 'guidance_itinerary'", 'Trip Builder should render the concise guidance itinerary as a bounded result section.');
assertIncludes(screen, "| 'plan_actions'", 'Trip Builder should render Offline, Save, and Navigate as one bounded result section.');
assertIncludes(screen, 'testID={`trip-builder-guidance-point-${point.role}`}', 'Trip Builder should expose the exact mounted guidance sequence.');
assertIncludes(screen, 'testID="trip-builder-prepare-offline-pack"', 'Trip Builder should expose the mounted Offline action.');
assertIncludes(screen, 'testID="trip-builder-save-route"', 'Trip Builder should expose the mounted Save action.');
assertIncludes(screen, 'testID="trip-builder-start-guidance"', 'Trip Builder should expose the mounted Navigate action.');
...
assertIncludes(screen, 'setTripSetupStarted(false);', 'Direct summary handoffs should wait for practical entry resolution and canonical preparation.');
assertIncludes(screen, "ready.status !== 'ready' || !ready.canonicalRoute", 'Only resolved canonical-ready sessions may enter setup.');
assertIncludes(screen, 'setPreparedTripRoutePreview(buildPreparedTripRoutePreview(persistedRoute));', 'Resolved canonical detail should prepare the setup preview.');
assertIncludes(screen, 'setTripSetupStarted(true);', 'Selected detail completion should open the requested Trip Builder setup.');
assertIncludes(screen, 'testID="trip-builder-route-preparation-state"', 'Route preparation should expose a visible async terminal state and retry action.');
assertIncludes(screen, 'routePreparationStateRef.current.requestId !== started.requestId', 'Selected detail readiness should reject stale request identities.');
assertIncludes(screen, "routePreparationState.status === 'ready'", 'Only canonical-ready preparation may enable setup.');
assertIncludes(screen, 'String(routePreparationState.routeId) === String(selectedRoute?.id)', 'A stale ready route must not enable a newer selection.');
assertIncludes(screen, 'const routeForSetup = resolvedSelectedRouteDetail;', 'Trip Builder setup should consume only matching validated canonical geometry.');
...
assertIncludes(screen, 'const selectedTrailheadResupplyAnchorCoordinate = useMemo(', 'Trip Builder should keep a stable trailhead endpoint available as the final Smart Resupply fallback.');
assertIncludes(screen, '() => remoteEntryForResupply(selectedTrailheadResupplyAnchorCoordinate)', 'Smart Resupply must receive the exact practical entry resolved by Trip Builder.');
assertIncludes(screen, "source: 'practical_trail_entry'", 'Smart Resupply should label the prepared endpoint as the practical trail-entry boundary.');
assertIncludes(screen, 'progressRatio: 1', 'Smart Resupply must not substitute an earlier remoteness/service-loss ratio for practical entry.');
...
assertIncludes(screen, 'left.remainingApproachMilesToTrailhead', 'Fuel and supply options should preserve remaining-distance-first order before title tie breakers.');
...
assertIncludes(screen, 'routeCoverageComplete: searchCoverage.complete', 'Smart Resupply should not treat under-sampled provider coverage as a valid empty result.');
...
assertIncludes(screen, 'Add fuel or supplies', 'Trip Builder itinerary edit mode should expose only supported guidance-stop insertion.');
assertIncludes(screen, 'Add another practical fuel, grocery, or supply stop before trail entry', 'Trip Builder itinerary add slots should match the canonical guidance roles.');
