...
const {
  NO_ACTIVE_CONVOY_COMMAND_VIEW_MODEL,
...
const noActive = selectConvoyCommandWidgetViewModel({ nowMs });
assert.deepStrictEqual(noActive, NO_ACTIVE_CONVOY_COMMAND_VIEW_MODEL);
...
const live = selectConvoyCommandWidgetViewModel({
  nowMs,
...
const alert = selectConvoyCommandWidgetViewModel({
  nowMs,
  commandData: {
    ...liveCommandData,
    dataState: 'live',
...
const estimated = selectConvoyCommandWidgetViewModel({
  nowMs,
  commandData: {
    ...liveCommandData,
    mode: 'planned',
...
assert.strictEqual(formatConvoyDistanceMiles(0), '<0.1 mi');
assert.strictEqual(formatConvoyDistanceMiles(0.02), '<0.1 mi');
...
console.log('[convoy-command-widget-selector] safe default, live, alert, estimated, and distance checks passed');
